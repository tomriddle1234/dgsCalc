    ...
    if (sublistCount == 5):
        ...
        if codeTable[-1] and encodeList[-1]:
            ...
            if currentIndex != 0:
                ...
                if checkBasenameBefore == filebasename:
                    print "same basename detected. %s" % filebasename
                    print encodeList
                    print codeTable[-1]
                    encodeList[-1] = codeTable[-1][1][3:-1]
                    print encodeList
        ...
        if (substrList[-1] != '' and substrList[-1] != ' ' and substrList[-1].isdigit()):
            ...
            if (int(substrList[-1]) == 3):
                encodeList.append('c')
            if (int(substrList[-1]) == 4):
                encodeList.append('d')
            if (int(substrList[-1]) == 5):
                encodeList.append('e')
            if (int(substrList[-1]) == 6):
                encodeList.append('f')
            if (int(substrList[-1]) == 7):
                encodeList.append('g')
            if (int(substrList[-1]) == 8):
                encodeList.append('h')
    
    codeTable.append([filebasename,''.join(encodeList)])

filebasenameList = []
for codePair in codeTable:
    preparedFilebasename = ""
    substrList = codePair[0].split('-')
    if len(substrList) == 5:
        preparedFilebasename = '-'.join(substrList[:-1])
    else:
        preparedFilebasename = codePair[0]
    filebasenameList.append(preparedFilebasename)
from collections import Counter
dupfilebnList = [k for (k,v) in Counter(filebasenameList).iteritems() if v > 1]
print dupfilebnList
for dupCode in dupfilebnList:
    encodeChange = ""
    for codePair in codeTable:
        if dupCode in codePair[0]:
            encodeChange = codePair[1]
            if encodeChange.endswith('a'):
                encodeChange = encodeChange[:-1]
            break
    for i in range(len(codeTable)):
        if dupCode in codeTable[i][0]:
            lastcharacter = codeTable[i][1][-1]
            if not lastcharacter.isdigit():
